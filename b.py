...
class Graph:
    ...
    def printExchangeRates(self, cycle):
        ...
        initial_value = 1.0  # Start with 1 unit of the initial currency
        value = initial_value
        ...
        for i in range(len(cycle) - 1):
            ...
            for edge_u, edge_v, weight in self.graph:
                if edge_u == u and edge_v == v:
                    ...
                    value *= exchange_rate  # Update value by multiplying by the exchange rate
                    print(f"{currency_u} -> {currency_v}: 1 {currency_u} = {exchange_rate:.6f} {currency_v} | Value now: {value:.6f} {currency_v}")
                    break

        # Final value after completing the cycle
