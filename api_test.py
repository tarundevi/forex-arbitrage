...
class Graph:
    ...
    def printNegativeCycle(self, parent, start):
        ...
        while current not in visited:
            visited.add(current)
            cycle.append(self.index_to_currency[current])  # Append currency name
            prev = parent[current]
        ...
        cycle_start = self.index_to_currency[current]
        cycle = cycle[cycle.index(cycle_start):] + [cycle_start]  # Complete the cycle

        print("Negative weight cycle found:", " -> ".join(cycle))
